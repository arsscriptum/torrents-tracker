...
BASE_URL = "https://www.ilcorsaronero.in"
SEARCH_LOC = "/argh.php?search=%s"
LIST_LOC = "/recenti"


class ilcorsaronero(object):
    """ ilcorsaronero module for we-get
    """
    ...
    def set_item(self, link):
        ...
        links = re.findall(r'a class=[\'"]+forbtn magnet[\'"]+ href=[\'"]+(.*?)[\'"]+', data, re.IGNORECASE)
        seeders = re.findall(r'seeds:.*\n.*?>(.*?)<', data)[0].strip()
        leechers = re.findall(r'leech:.*\n.*?>(.*?)<', data)[0].strip()
        item = dict()
        name = None
        for link in links:
            if "magnet" in link:
        ...
        if magnet == None:
            return item
        name = self.module.fix_name(self.module.magnet2name(magnet)).strip()
        item.update(
